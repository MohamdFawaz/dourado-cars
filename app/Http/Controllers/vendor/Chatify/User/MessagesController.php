...
namespace App\Http\Controllers\vendor\Chatify\User;
...
use App\Libs\Chatify\User\ChatifyMessenger as Chatify;
use App\Models\User;
...
class MessagesController extends Controller
{
    ...
    public function index( $id = null)
    {
        ...
        if (\auth()->guard('users')->check()) {
            // prepare id
            return view('Chatify::user.pages.app', [
                'id' => ($id == null) ? 0 : $route . '_' . $id,
                'route' => $route,
                'messengerColor' => Auth::guard('users')->user()->messenger_color,
                'dark_mode' => Auth::guard('users')->user()->dark_mode < 1 ? 'light' : 'dark',
            ]);
        } else {
            return view('vendor.Chatify.user.pages.login');
        }
    }
    ...
    public function idFetchData(Request $request)
    {
        ...
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch,
            'user_avatar' => asset('storage/users-avatar/avatar.png'),
        ]);
    }
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('admins',  function ($join) {
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', Auth::guard('users')->user()->id);
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = User::where('id', $favorite->favorite_id)->first();
            $favoritesList .= view('Chatify::user.layouts.favorite', [
                'user' => $user,
            ]);
        }
        // send the response
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->get() as $record) {
            $getRecords .= view('vendor.Chatify.user.layouts.listItem', [
                'get' => 'search_item',
                'type' => 'user',
                'user' => $record,
            ])->render();
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('vendor.Chatify.user.layouts.listItem', [
                'get' => 'sharedPhoto',
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? User::where('id', Auth::guard('users')->user()->id)->update(['dark_mode' => 1])  // Make Dark
                : User::where('id', Auth::guard('users')->user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request['messengerColor']) {
            ...
            User::where('id', Auth::guard('users')->user()->id)
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::guard('users')->user()->avatar != config('chatify.user_avatar.default')) {
                        $path = asset('storage/users-avatar/avatar.png');
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $update = User::where('id', Auth::guard('users')->user()->id)->update(['avatar' => $avatar]);
